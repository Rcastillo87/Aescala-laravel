    ...
    selectImporte.addEventListener('change', async () => {
        ...
        try {
            ...
            json.data.forEach(item => {
                ...
                // 🔥 Rellenar valores
                block.querySelector('[name$="[producto]"]').value = item.producto ?? '';
                block.querySelector('[name$="[valor_unidad]"]').value = item.valor_unidad ?? '';
                block.querySelector('[name$="[tipo]"]').value = item.tipo ?? '';
