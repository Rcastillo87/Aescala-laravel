             ...
             <form method="GET" action="{{route("user.index")}}">
                ...
                <div class="flex flex-wrap gap-1">
                    ...
                    <div class="p-2 shrink-0 w-[40]">
                        ...
                        <x-text-input id="cedula" class="block mt-1 w-full" type="number" name="cedula" :value="Request('cedula')" />
                    </div>
                    <div class="p-2 shrink-0 w-[40]">
                        <x-input-label for="telefono" :value="__('Telefono')" />
                        <x-text-input id="telefono" class="block mt-1 w-full" type="number" name="telefono" :value="Request('telefono')" />
                    </div>
                    <div class="p-2 shrink-0 w-[200px]">
                        ...
                        <select name="id_rol" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 
                            ...
                            @foreach ($roles as $key => $rol)
                                <option value="{{$key}}" {{(Request('id_rol') == $key)?'selected':'' }}>
                                    {{$rol}}</option>
                            @endforeach
                        </select>
                    </div>
