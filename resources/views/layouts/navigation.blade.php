<nav class="bg-gradient-to-br @if (env('APP_ENV') == 'production') from-blue-400 to-blue-200 @else from-orange-400 to-orange-200 @endif rounded-lg shadow-md shadow-gray-300 dark:bg-gray-800 dark:border-gray-700">
    <div class="flex flex-wrap items-center p-2 justify-between sm:justify-end">
        ...
        <div class="flex my-3 mr-2">
            <x-dropdown width="48">
                <x-slot name="trigger">
                    <button class="items-center px-3 py-2 space-y-1 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
    ...
    @session('error')
        <div class="p-2 text-sm text-right text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
            {{ session('error') }}
        </div>
    @endsession
    @session('success')
    <div class="p-2 text-sm text-right text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
        {{ session('success') }}
    </div>
    @endsession
    @session('warning')
    <div class="p-2 text-sm text-right text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
        {{ session('warning') }}
    </div>
    @endsession
  </nav>
