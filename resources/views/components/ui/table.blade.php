...
<div class="relative overflow-x-auto rounded-lg border border-gray-200">
    <table class="w-full text-sm text-gray-700 {{ $tableClass }}">
        <thead class="bg-orange-500 text-xs text-white uppercase rounded-lg">
            <tr>
                @foreach($columns as $col)
                    <th class="px-4 py-3 text-center {{ $headerClass[$col] ?? '' }}">
        ...
        <tbody>
            @forelse($data as $row)
                <tr class="border-b hover:bg-gray-50 {{ $rowClass }}">
                    @foreach($columns as $col)
                        <td class="px-4 py-2 text-center align-middle {{ $columnClass[$col] ?? '' }}">
