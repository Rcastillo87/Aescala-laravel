...
<form method="POST" id='formValorArea' action="{{ route('configuracion.saveAdicionales') }}">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select onchange="location = this.value"
                    ...
                    @foreach($años as $año)
                        <option
                            value="{{ route('configuracion.indexAdicionales', $año) }}"
            ...
            @if(empty($items->toArray()))
                <div>
                    ...
                    <select id="importe_año" data-type="adicionales"
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div class="border border-gray-200 rounded-lg p-3 bg-white space-y-2">
                ...
                <div class="flex flex-wrap -mx-3">
                    ...
                    <div class="w-full max-w-full px-3 py-1 shrink-0 md:w-6/12 lg:w-6/12 md:flex-0">
                        ...
                        <x-text-input class="block mt-1 w-full" type="text" name="items[{{ $loop->index }}][producto]" 
                            :value="old('items.' . $loop->index . '.producto', $item['producto'] ?? '')" required />
                    </div>

                    <div class="w-full max-w-full px-3 py-1 shrink-0 md:w-6/12 lg:w-4/12 2xl:w-3/12 md:flex-0">
                        <x-input-label :value="__('Valor Unidad *')" />
                        <x-text-input class="block mt-1 w-full" type="number" min="0" step="1" name="items[{{ $loop->index }}][valor_unidad]" 
                            :value="old('items.' . $loop->index . '.valor_unidad', $item['valor_unidad'] ?? '')" required />
                    </div>

                    <div class="w-full max-w-full px-3 py-1 shrink-0 md:w-6/12 lg:w-4/12 2xl:w-3/12 md:flex-0">
                        <x-input-label :value="__('Tipo *')" />
                        <input type="hidden" name="items[{{ $loop->index }}][tipo]" value="{{ $item['tipo'] ?? '' }}">
                        {!! $item['spanTipo'] !!}
                    </div>
    ...
    <div class="flex justify-between mt-4">
        <x-secondary-button id="btnAddItem" data-empy="{{ empty($items->toArray()) ? 1 : 0 }}" type="button">
            + Añadir intervalo
        </x-secondary-button>
        ...
        <div class="flex flex-wrap -mx-3">
            ...
            <div class="w-full max-w-full px-3 py-1 md:w-6/12 lg:w-4/12 2xl:w-3/12">
                ...
                <input type="number" min="0" step="1"
                    name="items[__INDEX__][valor_unidad]"
                    class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300
                           focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"
                    required>
            </div>
